...
public class NthPrime_12_SFPP {
    ...
    //public static int nthPrime(int n, ThreadPipe threadPipe) {
    public static int nthPrime(int n, ThreadPipe threadPipe) {
    	//Log.e("PIRME", "NthPrime : Inside nthPrime() -> " + n);
        if (n < 2) return 2;
        if (n == 2) return 3;
        ...
        for(candidate = 5, count = 2; count < n; step = 6-step, candidate += step) {
            ...
            //Log.v("PRIME", "NthPrime : count -> " + count 
            //							+ ", n -> " + n);
            if(threadPipe.isNotification() == true) {
            	//Log.v("PRIME", "NthPrime : nthPrime() : Notification received." 
            	//				+  "ABORTING calculation for " + n);
